def deep_len(lnk):
    ...
    if lnk.first == Link.empty:
        ...
        count = count + deep_len(lnk.first) + deep_len(lnk.rest)
        return count
...
def make_to_string(front, mid, back, empty_repr):
    ...
    def list_conversion(lnk):
        ...
        if lnk == Link.empty:
            ...
            return front + str(lnk.first) + mid + list_conversion(lnk.rest) + back
...
def add_trees(t1, t2):
    ...
    shared_branches = [add_trees(l,r) for l,r in zip(t1.branches,t2.branches)]
    if len(t2.branches) >= len(t1.branches):
        remainder = t2.branches[len(t1.branches):]
    else:
        remainder = t1.branches[len(t2.branches):]
    return Tree(root, shared_branches + remainder)
