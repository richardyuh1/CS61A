...
class Name(Expr):
    ...
    def eval(self, env):
        ...
        """
        "*** YOUR CODE HERE ***"
        if str(self) in env.keys():
            return env.get(str(self))
        else:
            raise NameError('Key does not exist in dictionary')
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        for i in range(len(self.parameters)):
                d[self.parameters[i]] = arguments[i]
